 ...
 if ((isset($_POST["agregar"]))&&($_POST["agregar"]=="formu"))
    {
        ...
        $validar="SELECT * FROM linea WHERE id_linea='$id' or linea='$linea'";
        $queryi=$conectar->prepare($validar);
        $queryi->execute();
        $fila1=$queryi->fetchAll(PDO::FETCH_ASSOC);

        if ($fila1){
            echo '<script> alert ("LA LINEA YA EXISTE");</script>';
        ...
        {
            $insertsql=$conectar->prepare("INSERT INTO linea(id_linea,linea) VALUES (?,?);");
            $insertsql->execute([$id,$linea]);
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="col-sm-8">
                    ...
                    <?php require_once "tabla_linea.php";?>
                    </div>
				</div>
			</div>
		</div>
