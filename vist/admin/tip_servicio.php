...
$conectar= $db->conectar();
 ...
 if ((isset($_POST["agregar"]))&&($_POST["agregar"]=="formu"))
    {
        $id=$_POST['idm'];
        $tip_servicio = $_POST['tip_servicio'];

        

        $validar="SELECT * FROM tipo_servicio WHERE id_tip_servicio ='$id' or tip_servicio ='$tip_servicio'";
        $queryi=$conectar->prepare($validar);
        $queryi->execute();
        $fila1=$queryi->fetchAll(PDO::FETCH_ASSOC);

        if ($fila1){
            echo '<script> alert ("EL TIPO DE SERVICIO YA EXISTE ");</script>';
            echo '<script> windows.location= "index.php"</script>';

        }

        else if ($id=="" || $tip_servicio =="" )
        {
        ...
        {
            $insertsql=$conectar->prepare("INSERT INTO tipo_servicio (id_tip_servicio,tip_servicio) VALUES (?,?);");
            $insertsql->execute([$id,$tip_servicio]);
			...
			<div class="row">
				<div class="col-sm-4">
					<form id="frmArticulos"  name="formu" method="post" >
						...
						<input type="text" oninput="multipletext(this)" maxlength="12" class="form-control input-sm" id="marca" name="tip_servicio">
						<br>
				...
				<div class="col-sm-8">
                    ...
                    <?php require_once "tabla_tip_servicio.php";?>
                    </div>
				</div>
			</div>
		</div>
