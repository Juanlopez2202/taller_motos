...
$conectar= $db->conectar();
 ...
 if ((isset($_POST["agregar"]))&&($_POST["agregar"]=="formu"))
    {
        $id=$_POST['idm'];
        $doculegal = $_POST['doculegal'];
        $precio = $_POST['precio'];

        

        $validar="SELECT * FROM documentos WHERE id_documentos ='$id' or documentos ='$doculegal'";
        $queryi=$conectar->prepare($validar);
        $queryi->execute();
        $fila1=$queryi->fetchAll(PDO::FETCH_ASSOC);

        if ($fila1){
            echo '<script> alert ("EL DOCUMENTO LEGAL YA EXISTE ");</script>';
            echo '<script> windows.location= "index.php"</script>';

        }

        else if ($id=="" || $doculegal =="" || $precio =="")
        {
        ...
        {
            $insertsql=$conectar->prepare("INSERT INTO documentos (id_documentos,documentos,precio) VALUES (?,?,?);");
            $insertsql->execute([$id,$doculegal,$precio]);
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="col-sm-8">
                    ...
                    <?php require_once "tabla_doculegal.php";?>
                    </div>
				</div>
			</div>
		</div>
