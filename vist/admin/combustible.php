 ...
 if ((isset($_POST["agregar"]))&&($_POST["agregar"]=="formu"))
    {
        ...
        $validar="SELECT * FROM combustible WHERE id_combustible ='$id' or combustible ='$combustible'";
        $queryi=$conectar->prepare($validar);
        $queryi->execute();
        $fila1=$queryi->fetchAll(PDO::FETCH_ASSOC);

        if ($fila1){
            echo '<script> alert ("EL COMBUSTIBLE YA EXISTE ");</script>';
        ...
        {
            $insertsql=$conectar->prepare("INSERT INTO combustible (id_combustible,combustible) VALUES (?,?);");
            $insertsql->execute([$id,$combustible]);
			...
			<div class="row">
				<div class="col-sm-4">
					<form id="frmArticulos"  name="formu" method="post" >
						...
						<input type="text" oninput="multipletext(this)" maxlength="12" class="form-control input-sm" id="marca" name="combustible">
						<br>
				...
				<div class="col-sm-8">
                    ...
                    <?php require_once "tabla_combustible.php";?>
                    </div>
				</div>
			</div>
		</div>
