...
?>
   <?php
        ...
        $consulta=$conectar->prepare("SELECT * from color where id_color ='".$_GET['actu']."' ");
             $consulta->execute();
       ...
       if ((isset($_POST["actualizar"]))&&($_POST["actualizar"]=="form"))
       {
        ...
        {
          $actusql=$conectar->prepare("UPDATE color SET color ='$nombre' WHERE id_color ='".$_GET['actu']."'");
          $actusql->execute();
			...
			<div class="row">
				<div class="col-sm-4">
					<form id="frmArticulos"  name="formu" method="post" >
						...
						<input type="number" disabled class="form-control input-sm" id="id" name="idu"   value="<?php echo $query['id_color'] ?>">

						<label>Color</label>
						<input type="text" class="form-control input-sm" id="nombre" name="nombreu"  value="<?php echo $query['color'] ?>">
